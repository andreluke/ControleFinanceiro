...
const __copyProps = (to, from, except, desc5) => {
		...
		for (const key of __getOwnPropNames(from))
			if (!__hasOwnProp.call(to, key) && key !== except)
				__defProp(to, key, {
					get: () => from[key],
					enumerable:
						!(desc5 = __getOwnPropDesc(from, key)) || desc5.enumerable,
				});
	}
	return to;
};
	...
	"node_modules/.pnpm/dotenv@17.3.1/node_modules/dotenv/lib/main.js"(
		...
		function configDotenv(options) {
			...
			let debug = parseBoolean(
				processEnv.DOTENV_CONFIG_DEBUG || options?.debug,
			);
...
const import_jwt = __toESM(require("@fastify/jwt"), 1);
const import_swagger = __toESM(require("@fastify/swagger"), 1);
const import_swagger_ui = __toESM(require("@fastify/swagger-ui"), 1);
...
const errorMessages = {
	...
	AUTH_UNAUTHORIZED:
		"Voc\xEA precisa estar autenticado para acessar este recurso.",
	...
	if (err.name === "ZodError" || err instanceof import_zod.ZodError) {
		...
		if (firstError) {
			...
			if (firstError.code === "invalid_type") {
				...
				const validOptions =
					firstError.options?.join(", ") || "valores v\xE1lidos";
...
const import_drizzle_orm = require("drizzle-orm");

// src/drizzle/client.ts
const import_postgres_js = require("drizzle-orm/postgres-js");
const import_postgres = __toESM(require("postgres"), 1);

// src/settings/env.ts
const import_zod2 = require("zod");
const envSchema = import_zod2.z.object({
...
const import_pg_core = require("drizzle-orm/pg-core");
const transactionTypeEnum = (0, import_pg_core.pgEnum)("transaction_type", [
	"income",
	"expense",
]);
const frequencyEnum = (0, import_pg_core.pgEnum)("frequency_type", [
	"daily",
	"weekly",
	"monthly",
	"yearly",
...
const budgetPeriodEnum = (0, import_pg_core.pgEnum)("budget_period", [
	"monthly",
]);
...
const categories = (0, import_pg_core.pgTable)("categories", {
	id: (0, import_pg_core.uuid)("id").primaryKey().defaultRandom(),
...
const subcategories = (0, import_pg_core.pgTable)("subcategories", {
	id: (0, import_pg_core.uuid)("id").primaryKey().defaultRandom(),
	userId: (0, import_pg_core.uuid)("user_id")
		.references(() => users.id, { onDelete: "cascade" })
		.notNull(),
	categoryId: (0, import_pg_core.uuid)("category_id")
...
const paymentMethods = (0, import_pg_core.pgTable)("payment_methods", {
	id: (0, import_pg_core.uuid)("id").primaryKey().defaultRandom(),
	userId: (0, import_pg_core.uuid)("user_id")
		.references(() => users.id, { onDelete: "cascade" })
		.notNull(),
...
const transactions = (0, import_pg_core.pgTable)("transactions", {
	id: (0, import_pg_core.uuid)("id").primaryKey().defaultRandom(),
	userId: (0, import_pg_core.uuid)("user_id")
		.references(() => users.id, { onDelete: "cascade" })
		.notNull(),
	categoryId: (0, import_pg_core.uuid)("category_id").references(
		() => categories.id,
	),
	subcategoryId: (0, import_pg_core.uuid)("subcategory_id").references(
		() => subcategories.id,
	),
...
const recurringTransactions = (0, import_pg_core.pgTable)(
...
const budgets = (0, import_pg_core.pgTable)("budgets", {
	id: (0, import_pg_core.uuid)("id").primaryKey().defaultRandom(),
	userId: (0, import_pg_core.uuid)("user_id")
		.references(() => users.id, { onDelete: "cascade" })
		.notNull(),
	...
	baseAmount: (0, import_pg_core.numeric)("base_amount", {
		precision: 12,
		scale: 2,
	}),
	...
	isRecurring: (0, import_pg_core.boolean)("is_recurring")
		.notNull()
		.default(false),
	isActive: (0, import_pg_core.boolean)("is_active").notNull().default(true),
...
const goals = (0, import_pg_core.pgTable)("goals", {
	id: (0, import_pg_core.uuid)("id").primaryKey().defaultRandom(),
	userId: (0, import_pg_core.uuid)("user_id")
		.references(() => users.id, { onDelete: "cascade" })
		.notNull(),
	...
	targetAmount: (0, import_pg_core.numeric)("target_amount", {
		precision: 12,
		scale: 2,
	}).notNull(),
	currentAmount: (0, import_pg_core.numeric)("current_amount", {
		precision: 12,
		scale: 2,
	})
		.notNull()
...
const goalContributions = (0, import_pg_core.pgTable)("goal_contributions", {
	id: (0, import_pg_core.uuid)("id").primaryKey().defaultRandom(),
	goalId: (0, import_pg_core.uuid)("goal_id")
		.references(() => goals.id, { onDelete: "cascade" })
		.notNull(),
	transactionId: (0, import_pg_core.uuid)("transaction_id")
...
// src/drizzle/client.ts
const client = (0, import_postgres.default)(env.DATABASE_URL);
...
const BudgetsModel = class {
	async recalculateParentBudget(userId, categoryId, month, year) {
		...
		if (parentBudget) {
			...
			await db
				...
				.where((0, import_drizzle_orm.eq)(budgets.id, parentBudget.id));
		}
	}
	async create(data) {
		...
		if (data.isRecurring) {
			...
			await db
				...
				.where((0, import_drizzle_orm.eq)(budgets.id, created.id));
		}
		if (isSubcategory) {
	...
	async findById(id) {
		const [budget] = await db
			.select()
			.from(budgets)
			.where((0, import_drizzle_orm.eq)(budgets.id, id));
		return budget;
	}
	...
	async findByUser(userId) {
		return db
			.select()
			.from(budgets)
			.where((0, import_drizzle_orm.eq)(budgets.userId, userId))
			.orderBy(
				(0, import_drizzle_orm.desc)(budgets.year),
				(0, import_drizzle_orm.desc)(budgets.month),
			);
	}
	...
	async ensureRecurringBudgetsExist(userId, month, year) {
		...
		for (const groupId of missingGroupIds) {
			...
			const [isActive] = await db
				...
				.from(budgets)
				.where((0, import_drizzle_orm.eq)(budgets.id, groupId))
				.limit(1);
	...
	async getWithCategory(userId, month, year) {
		...
		const userBudgets = await db
			...
			.leftJoin(
				categories,
				(0, import_drizzle_orm.eq)(budgets.categoryId, categories.id),
			)
			.leftJoin(
				subcategories,
				(0, import_drizzle_orm.eq)(budgets.subcategoryId, subcategories.id),
			)
			.where(
				(0, import_drizzle_orm.and)(
					(0, import_drizzle_orm.eq)(budgets.userId, userId),
		...
		const budgetsWithSpent = await Promise.all(
			userBudgets.map(async (budget) => {
				...
				const [result] = await db
					.select({ total: (0, import_drizzle_orm.sum)(transactions.amount) })
					.from(transactions)
					.where((0, import_drizzle_orm.and)(...conditions));
	...
	async getSubcategoriesTotal(
		...
		return categoryBudgets
			...
			.reduce((sum2, b) => sum2 + toNumber(b.amount), 0);
	}
	async toggleActive(id, userId) {
		...
		await db
			.update(budgets)
			.set({ isActive: newIsActive, updatedAt: /* @__PURE__ */ new Date() })
			.where((0, import_drizzle_orm.eq)(budgets.id, id));
...
const budgets_model_default = new BudgetsModel();

// src/modules/budgets/budgets.schema.ts
const import_zod3 = require("zod");
const createBudgetSchema = import_zod3.z.object({
	categoryId: import_zod3.z.string().uuid("ID da categoria inv\xE1lido"),
	subcategoryId: import_zod3.z
		.string()
		.uuid("ID da subcategoria inv\xE1lido")
		.optional(),
...
const updateBudgetSchema = import_zod3.z.object({
...
const toggleBudgetActiveSchema = import_zod3.z.object({
	isActive: import_zod3.z.boolean(),
});
const budgetQuerySchema = import_zod3.z.object({
...
const budgets_controller_default = new BudgetsController();
...
const import_drizzle_orm5 = require("drizzle-orm");

// src/modules/categories/categories.model.ts
const import_drizzle_orm2 = require("drizzle-orm");
const CategoryModel = class {
	async findAll(userId) {
		return db
			...
			.orderBy((0, import_drizzle_orm2.asc)(categories.name));
	}
...
const import_drizzle_orm3 = require("drizzle-orm");
const PaymentMethodModel = class {
	async findAll(userId) {
		return db
			.select({
...
const import_drizzle_orm4 = require("drizzle-orm");
const TransactionModel = class {
	async findAll(userId, filters) {
		...
		const query = db
			...
			.leftJoin(
				categories,
				(0, import_drizzle_orm4.eq)(transactions.categoryId, categories.id),
			)
			.leftJoin(
				subcategories,
				(0, import_drizzle_orm4.eq)(
					transactions.subcategoryId,
					subcategories.id,
				),
			)
			...
			.where(
				(0, import_drizzle_orm4.and)(
					...
					filters.type
						? (0, import_drizzle_orm4.eq)(transactions.type, filters.type)
						: void 0,
					filters.categoryId
						? (0, import_drizzle_orm4.eq)(
								transactions.categoryId,
					...
					filters.paymentMethodId
						...
						: void 0,
					filters.month
						? import_drizzle_orm4.sql`to_char(${transactions.date}, 'YYYY-MM') = ${filters.month}`
						: void 0,
					startDate
						? (0, import_drizzle_orm4.gte)(transactions.date, startDate)
						: void 0,
					endDate
						? (0, import_drizzle_orm4.lte)(transactions.date, endDate)
						: void 0,
				),
			)
			.orderBy(
				(0, import_drizzle_orm4.desc)(transactions.date),
				(0, import_drizzle_orm4.desc)(transactions.createdAt),
			)
			.limit(filters.limit)
			.offset((filters.page - 1) * filters.limit);
		const countQuery = db
			.select({ count: (0, import_drizzle_orm4.count)() })
			.from(transactions)
			.where(
				(0, import_drizzle_orm4.and)(
					(0, import_drizzle_orm4.eq)(transactions.userId, userId),
	...
	async findById(id, userId) {
		const [transaction] = await db
			...
			.where(
				(0, import_drizzle_orm4.and)(
					(0, import_drizzle_orm4.eq)(transactions.id, id),
					(0, import_drizzle_orm4.eq)(transactions.userId, userId),
				),
			)
			.limit(1);
...
const GoalsModel = class {
	...
	async findAll(userId) {
		const result = await db
			...
			.from(goals)
			.where((0, import_drizzle_orm5.eq)(goals.userId, userId))
	...
	async findContributionsByGoalId(goalId) {
		const result = await db
			...
			.where((0, import_drizzle_orm5.eq)(goalContributions.goalId, goalId))
	...
	async removeContribution(userId, contributionId) {
		const [contribution] = await db
			.select()
			.from(goalContributions)
			.where((0, import_drizzle_orm5.eq)(goalContributions.id, contributionId))
			.limit(1);
		...
		await db
			...
			.where((0, import_drizzle_orm5.eq)(goals.id, goal.id));
		await db
...
const createGoalSchema = import_zod4.z.object({
...
const updateGoalSchema = import_zod4.z.object({
	name: import_zod4.z.string().min(1, "Nome \xE9 obrigat\xF3rio").max(255),
	description: import_zod4.z.string().max(500).optional(),
	targetAmount: import_zod4.z.number().positive("Valor alvo deve ser positivo"),
	deadline: import_zod4.z.string().datetime().optional(),
...
const contributeGoalSchema = import_zod4.z.object({
	amount: import_zod4.z.number().positive("Valor deve ser positivo"),
});
const withdrawGoalSchema = import_zod4.z.object({
	amount: import_zod4.z.number().positive("Valor deve ser positivo"),
});

// src/modules/goals/goals.controller.ts
const GoalsController = class {
...
const goals_controller_default = new GoalsController();
...
const import_drizzle_orm6 = require("drizzle-orm");
const SubcategoryModel = class {
	async findAll(userId) {
		return db
			.select({
				id: subcategories.id,
			...
			.leftJoin(
				categories,
				(0, import_drizzle_orm6.eq)(subcategories.categoryId, categories.id),
			)
			.where(
...
const SubcategoriesController = class {
...
const subcategories_controller_default = new SubcategoriesController();

// src/modules/subcategories/subcategories.routes.ts
const PUBLIC_ROUTES2 = ["/health", "/docs"];
...
const import_drizzle_orm7 = require("drizzle-orm");
const import_exceljs = __toESM(require("exceljs"), 1);
const import_pdfkit = __toESM(require("pdfkit"), 1);
const DEFAULT_COLOR = "FF607D8B";
const DashboardExportModel = class {
	getDateRange(filters) {
		const startDate = filters.startDate
			? /* @__PURE__ */ new Date(`${filters.startDate}T00:00:00`)
			: void 0;
		const endDate = filters.endDate
			? /* @__PURE__ */ new Date(`${filters.endDate}T23:59:59`)
			: void 0;
	...
	async getCategoryColors(userId) {
		const userCategories = await db
			...
			.from(categories)
			.where((0, import_drizzle_orm7.eq)(categories.userId, userId));
	...
	async getTransactions(userId, filters) {
		...
		const rows = await db
			...
			.leftJoin(
				categories,
				(0, import_drizzle_orm7.eq)(transactions.categoryId, categories.id),
			)
			.leftJoin(
				paymentMethods,
				(0, import_drizzle_orm7.eq)(
					transactions.paymentMethodId,
					paymentMethods.id,
				),
			)
			.where(
				(0, import_drizzle_orm7.and)(
					...
					filters.type
						? (0, import_drizzle_orm7.eq)(transactions.type, filters.type)
						: void 0,
					filters.categoryId
						? (0, import_drizzle_orm7.eq)(
								transactions.categoryId,
						...
						: void 0,
					startDate
						? (0, import_drizzle_orm7.gte)(transactions.date, startDate)
						: void 0,
					endDate
						? (0, import_drizzle_orm7.lte)(transactions.date, endDate)
						: void 0,
				),
			)
	...
	async exportToCsv(userId, filters) {
		...
		lines.push(
			"Data;Descri\xE7\xE3o;Categoria;M\xE9todo de Pagamento;Tipo;Valor",
		);
	...
	renderPdfSummary(doc, summary) {
		...
		doc
			.fontSize(10)
			.fillColor("#666666")
			.text(`Total de transa\xE7\xF5es: ${summary.transactionCount}`);
		doc.moveDown(2);
	}
...
${cat.percentage.toFixed(1)}%`,
	...
	renderPdfTransactions(doc, transactions2) {
		...
		doc.text("Descri\xE7\xE3o", 45 + colWidths[0], tableTop, {
			width: colWidths[1],
		});
...
const dashboard_export_model_default = new DashboardExportModel();

// src/modules/seed/dashboard.seed.controller.ts
const DashboardSeedController = class {
	/**
	...
	async exportExcel(req, reply) {
		...
		const buffer = await dashboard_export_model_default.exportToExcel(
			userId,
			filters,
		);
		reply.type(
	...
	async exportPdf(req, reply) {
		...
		const userId = req.user.sub;
		const filters = this.parseFilters(req.query);
		const buffer = await dashboard_export_model_default.exportToPdf(
			userId,
			filters,
		);
	...
	async exportCsv(req, reply) {
		...
		const userId = req.user.sub;
		const filters = this.parseFilters(req.query);
		const csvContent = await dashboard_export_model_default.exportToCsv(
			userId,
			filters,
		);
...
const dashboard_seed_controller_default = new DashboardSeedController();
...
async function registerSeedDashboardRoutes(app) {
	app.post(
		...
		dashboard_seed_controller_default.importFromExcel.bind(
			dashboard_seed_controller_default,
		),
	);
	app.get(
		...
		dashboard_seed_controller_default.exportExcel.bind(
			dashboard_seed_controller_default,
		),
	);
	app.get(
		...
		dashboard_seed_controller_default.exportPdf.bind(
			dashboard_seed_controller_default,
		),
	);
	app.get(
		...
		dashboard_seed_controller_default.exportCsv.bind(
			dashboard_seed_controller_default,
		),
	);
}
...
const import_bcryptjs = __toESM(require("bcryptjs"), 1);
const import_drizzle_orm8 = require("drizzle-orm");
...
const AuthModel = class {
	async findByEmail(email) {
		const [user] = await db
			.select()
			.from(users)
			.where((0, import_drizzle_orm8.eq)(users.email, email))
			.limit(1);
		return user;
	}
	async findById(id) {
		const [user] = await db
			...
			.from(users)
			.where((0, import_drizzle_orm8.eq)(users.id, id))
			.limit(1);
		return user;
	}
...
const createCategorySchema = import_zod7.z.object({
...
const CategoriesController = class {
...
const PUBLIC_ROUTES3 = ["/health", "/docs"];
...
const createPaymentMethodSchema = import_zod8.z.object({
...
const PaymentMethodsController = class {
	...
	updatePaymentMethod = async (request, reply) => {
		...
		if (err)
			throw new AppError("Erro ao atualizar m\xE9todo de pagamento", 500);
		if (!updated) {
			throw new AppError(
	...
	deletePaymentMethod = async (request, reply) => {
		...
		return reply.send({
			message: "M\xE9todo de pagamento deletado com sucesso",
		});
	};
	restorePaymentMethod = async (request, reply) => {
		...
		if (err)
			throw new AppError("Erro ao restaurar m\xE9todo de pagamento", 500);
		if (!restored) {
...
const import_drizzle_orm9 = require("drizzle-orm");
const RecurringTransactionModel = class {
	async findAll(userId, filters) {
		let conditions = (0, import_drizzle_orm9.eq)(
		...
		if (filters?.isActive !== void 0) {
			conditions = (0, import_drizzle_orm9.and)(
				conditions,
				(0, import_drizzle_orm9.eq)(
					recurringTransactions.isActive,
					filters.isActive,
				),
			);
		}
		...
		return db
			...
			.where(conditions)
			.orderBy((0, import_drizzle_orm9.asc)(recurringTransactions.createdAt));
	}
	async findById(id, userId) {
	...
	async updateLastGeneratedAt(id, userId, date) {
		const [updated] = await db
			.update(recurringTransactions)
			.set({ lastGeneratedAt: date, updatedAt: /* @__PURE__ */ new Date() })
			.where(
				(0, import_drizzle_orm9.and)(
					(0, import_drizzle_orm9.eq)(recurringTransactions.id, id),
					(0, import_drizzle_orm9.eq)(recurringTransactions.userId, userId),
...
const recurringTransactionBaseSchema = import_zod9.z.object({
	description: import_zod9.z
		.string()
	...
	categoryId: import_zod9.z
		.string()
		.uuid("ID de categoria inv\xE1lido")
		.optional(),
	subcategoryId: import_zod9.z
		.string()
		.uuid("ID de subcategoria inv\xE1lido")
		.optional(),
	paymentMethodId: import_zod9.z
		.string()
		.uuid("ID de m\xE9todo de pagamento inv\xE1lido")
		.optional(),
	frequency: import_zod9.z.enum(
		["daily", "weekly", "monthly", "yearly", "custom"],
		{
			errorMap: () => ({ message: "Frequ\xEAncia inv\xE1lida" }),
		},
	),
	customIntervalDays: import_zod9.z
		.number()
		.min(1, "Intervalo deve ser pelo menos 1 dia")
		.max(365, "Intervalo m\xE1ximo \xE9 365 dias")
		.optional(),
	dayOfMonth: import_zod9.z
		.number()
		.min(1, "Dia do m\xEAs deve ser entre 1 e 31")
		.max(31, "Dia do m\xEAs deve ser entre 1 e 31"),
	...
	endDate: import_zod9.z
		.string()
		.datetime({ message: "Data final deve ser ISO 8601 v\xE1lida" })
		.optional(),
});
const createRecurringTransactionSchema = recurringTransactionBaseSchema.refine(
	(data) => {
	...
	{
		message:
			"Intervalo personalizado \xE9 obrigat\xF3rio para frequ\xEAncia custom",
		path: ["customIntervalDays"],
	},
);
const updateRecurringTransactionSchema =
	recurringTransactionBaseSchema.partial();
const listRecurringTransactionsSchema = import_zod9.z.object({
...
const RecurringTransactionController = class {
	constructor(
	...
	getRecurringTransaction = async (request, reply) => {
		...
		if (err)
			throw new AppError("Erro ao buscar transa\xE7\xE3o recorrente", 500);
		if (!recurring) {
	...
	createRecurringTransaction = async (request, reply) => {
		...
		if (body.paymentMethodId) {
			...
			if (errPm)
				throw new AppError("Erro ao validar m\xE9todo de pagamento", 500);
			if (!method)
				throw new AppError("M\xE9todo de pagamento n\xE3o encontrado", 404);
		}
	...
	toggleRecurringTransaction = async (request, reply) => {
		...
		if (err)
			throw new AppError("Erro ao alternar transa\xE7\xE3o recorrente", 500);
		if (!toggled) {
			throw new AppError("Transa\xE7\xE3o recorrente n\xE3o encontrada", 404);
		}
	...
	processRecurringTransaction = async (request, reply) => {
		...
		if (!nextDate) {
			throw new AppError(
				"N\xE3o h\xE1 pr\xF3xima data para gerar transa\xE7\xE3o",
				400,
			);
		}
...
async function registerRecurringRoutes(app) {
	...
	app.get(
		"/recurring",
		{
			schema: {
				description:
					"Lista todas as transa\xE7\xF5es recorrentes do usu\xE1rio",
				tags: ["Recurring"],
				security: [{ bearerAuth: [] }],
				querystring: {
					type: "object",
...
const import_drizzle_orm10 = require("drizzle-orm");
...
const SummaryModel = class {
		...
		const [metaCategory] = await db
			...
			.where(
				(0, import_drizzle_orm10.and)(
					(0, import_drizzle_orm10.eq)(categories.userId, userId),
					(0, import_drizzle_orm10.eq)(categories.name, "Meta"),
				),
			)
			.limit(1);
	...
	async getSummary(userId, filters = {}) {
		...
		const balanceConditionsWithFilter = metaCategoryId
			? (0, import_drizzle_orm10.and)(
					balanceConditions,
					(0, import_drizzle_orm10.ne)(transactions.categoryId, metaCategoryId),
				)
			: balanceConditions;
	...
	async getMonthlySummary(userId) {
		...
		const result = await db
			...
			.groupBy(
				import_drizzle_orm10.sql`to_char(${transactions.date}, 'YYYY-MM')`,
			)
			.orderBy(
				import_drizzle_orm10.sql`to_char(${transactions.date}, 'YYYY-MM')`,
			);
	...
	async getByCategorySummary(userId, filters = {}) {
		...
		const expenses = await db
			...
			.leftJoin(
				categories,
				(0, import_drizzle_orm10.eq)(transactions.categoryId, categories.id),
			)
			.where(conditionsWithFilter)
			.groupBy(transactions.categoryId, categories.name, categories.color)
			.orderBy(
				(0, import_drizzle_orm10.desc)(
					import_drizzle_orm10.sql`SUM(${transactions.amount})`,
				),
			);
...
const summaryPeriodSchema = import_zod10.z.enum([
	"7d",
	"30d",
	"month",
	"previous",
]);
const summaryQuerySchema = import_zod10.z.object({
	month: import_zod10.z
		.string()
		.regex(/^\d{4}-\d{2}$/, "Formato de m\xEAs inv\xE1lido (esperado: YYYY-MM)")
		.optional(),
			...
			schema: {
				...
				querystring: {
					...
					properties: {
						month: {
							type: "string",
							description: "M\xEAs espec\xEDfico (YYYY-MM)",
						},
					},
				},
			},
...
const createTransactionSchema = import_zod11.z.object({
	description: import_zod11.z
		.string()
		.min(1, "Descri\xE7\xE3o \xE9 obrigat\xF3ria")
		.max(120),
	...
	categoryId: import_zod11.z
		.string()
		.uuid("ID de categoria inv\xE1lido")
		.optional(),
	subcategoryId: import_zod11.z
		.string()
		.uuid("ID de subcategoria inv\xE1lido")
		.optional(),
	paymentMethodId: import_zod11.z
		.string()
		.uuid("ID de m\xE9todo de pagamento inv\xE1lido")
		.optional(),
});
const updateTransactionSchema = createTransactionSchema.partial();
const listTransactionsSchema = import_zod11.z.object({
	month: import_zod11.z
		.string()
		.regex(/^\d{4}-\d{2}$/, "Formato de m\xEAs inv\xE1lido (esperado: YYYY-MM)")
		.optional(),
	...
	startDate: import_zod11.z
		.string()
		.datetime({ message: "startDate deve ser ISO 8601 v\xE1lida" })
		.optional(),
	endDate: import_zod11.z
		.string()
		.datetime({ message: "endDate deve ser ISO 8601 v\xE1lida" })
		.optional(),
...
const TransactionsController = class {
	constructor(
		transactionModel2 = new TransactionModel(),
		categoryModel = new CategoryModel(),
		paymentMethodModel = new PaymentMethodModel(),
