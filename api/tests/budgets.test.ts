...
describe("Budgets Module", () => {
  ...
  describe("BudgetsModel - create", () => {
    ...
    it("deve criar uma subcategoria e recalcular o pai", async () => {
      ...
      const parentBudgets = await budgetsModel.findByUserAndPeriod(userId, 3, 2026);
  ...
  describe("BudgetsModel - update", () => {
    it("deve editar o amount do orçamento", async () => {
      ...
      const updated = await budgetsModel.update(budget.id, userId, { amount: 250 });
    ...
    it("deve editar subcategoria e recalcular pai", async () => {
      ...
      const parentBudgets = await budgetsModel.findByUserAndPeriod(userId, 8, 2026);
      const parent = parentBudgets.find((b) => !b.subcategoryId && b.categoryId === newCat.id);
      expect(parent).toBeDefined();
    });

    it("deve retornar undefined para orçamento inexistente", async () => {
      const result = await budgetsModel.update("9f3c7a2e-6d1b-4f8a-9c52-3e7d1b8a6f0c", userId, { amount: 100 });
  ...
  describe("BudgetsModel - delete", () => {
    it("deve deletar orçamento e recalcular pai quando subcategoria é deletada", async () => {
      ...
      const parentBudgets = await budgetsModel.findByUserAndPeriod(userId, 9, 2026);
      const parent = parentBudgets.find((b) => !b.subcategoryId && b.categoryId === newCat.id);
  ...
  describe("BudgetsModel - getSummary", () => {
    it("deve calcular totais corretamente", async () => {
      ...
      expect(summary.totalRemaining).toBe(summary.totalBudgeted - summary.totalSpent);
    });
