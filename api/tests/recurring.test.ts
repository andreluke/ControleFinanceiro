...
describe('Recurring Transactions Module', () => {
  ...
  beforeAll(async () => {
    ...
    const category = await categoryModel.createCategory(userId, { name: 'Subscriptions' })
    categoryId = category.id

    const method = await methodModel.createMethod(userId, { name: 'Credit Card' })
    paymentMethodId = method.id
  })

  describe('RecurringTransactionModel', () => {
    it('should create a recurring transaction', async () => {
      const recurring = await recurringModel.createRecurringTransaction(userId, {
    ...
    it('should update a recurring transaction', async () => {
      ...
      const updated = await recurringModel.updateRecurringTransaction(created.id, userId, {
        amount: 120,
    ...
    it('should filter by isActive', async () => {
      ...
      const activeOnly = await recurringModel.findAll(userId, { isActive: true })
      const inactiveOnly = await recurringModel.findAll(userId, { isActive: false })
    ...
    it('should filter by type', async () => {
      ...
      const expenses = await recurringModel.findAll(userId, { type: 'expense' })
